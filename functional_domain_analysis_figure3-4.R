...
#arrange everything according to gig cluster and chromosomal arrangement 
g1cpf %>% arrange(cluster,chrom)
...
palette <- c("#fca311","#a7c957","#4f772d","#fb6f92","#bc6c25","#48cae4","#c77dff","#d00000","grey","yellow3")
...
g1gn<-clusters %>% filter(gfam=="gig1") %>% distinct(ggroup,gclade,chrom) %>% arrange(gclade,chrom)
...
#gig2
g2gn<-clusters %>% filter(gfam=="gig2") %>% distinct(ggroup,gclade,chrom) %>% arrange(gclade,chrom)
...
#Figure 4 components 
dtpg<-ssag2gn %>% filter(seq_id=="gig2B_ssa02d" | seq_id=="gig2B_ssa12a" | seq_id=="gig2B_ssa12b")
dtps<-ssag2s %>% filter(seq_id=="gig2B_ssa02d" | seq_id=="gig2B_ssa12a" | seq_id=="gig2B_ssa12b")

gggenomes(genes = dtpg,seqs=dtps) + geom_seq() + geom_gene(aes(fill=name),position="pile") +
  geom_bin_label() +  scale_fill_manual(values = palette) + labs(fill="Predicted functional clusters") 
